...
write_ascii(std::ostream& _out, BaseExporter& _be, Options _opt) const
{
  ...
  for (i=0, nV=_be.n_vertices(); i<nV; ++i)
  {
	...
	_out << " " << static_cast<unsigned int>(cA[0]) << " " 
	     << static_cast<unsigned int>(cA[1]) << " " 
	     << static_cast<unsigned int>(cA[2]) << " " 
	     << static_cast<unsigned int>(cA[3]);
      }else{
        //without alpha
        c  = _be.color(vh);
        _out << " " << static_cast<unsigned int>(c[0]) << " " 
...
write_binary(std::ostream& _out, BaseExporter& _be, Options _opt) const
{
  ...
  for (i=0, nV=_be.n_vertices(); i<nV; ++i)
  {
	...
	*/
        writeValue( ValueTypeUINT8, _out, (unsigned char)c[0]);
        writeValue( ValueTypeUINT8, _out, (unsigned char)c[1]);
        writeValue( ValueTypeUINT8, _out, (unsigned char)c[2]);

        if ( _opt.color_has_alpha() )
          // writeValue(ValueTypeINT32, _out, (int)c[3]);
	  writeValue(ValueTypeUINT8, _out, (unsigned char)c[3]);
    }
  }

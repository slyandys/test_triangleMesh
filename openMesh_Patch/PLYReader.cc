...
bool _PLYReader_::read(std::istream& _in, BaseImporter& _bi, Options& _opt) {
    ...
    // Binary reading depends on file not on user choices or mesh properties
    binary_file(_in); // sets/unsets options_::Binary
    ...
    // std::cerr << "Set all options" << std::endl;
    
    bool res = options_.is_binary() ? read_binary(_in, _bi, swap) : read_ascii(_in, _bi);
...
bool _PLYReader_::read_binary(std::istream& _in, BaseImporter& _bi, bool /*_swap*/) const {
  // std::cerr << "[PLYReader] : read binary file format" << std::endl;
    ...
    std::cerr << "Start of binary read" << std::endl;
    ...
    // std::cerr << "End of binary vertex read" << std::endl;
    ...
    // std::cerr << "End of binary read" << std::endl;
    return true;
}
...
bool _PLYReader_::binary_file(std::istream& _is) const {
    // save start position
    ...
    //Check if this file is really a ply format -- will be handled later
    ...
    if (fileType == "ascii") {
        options_ -= Options::Binary;
	       ...
	       fileType == "binary_big_endian") {
        options_ += Options::Binary;
    } else {
        // error will be handled lated
...
bool _PLYReader_::can_u_read(std::istream& _is) const {
    ...
    while (keyword != "end_header") {
      ...
      // std::cerr << keyword << " " << std::endl;
        ...
        if (keyword == "comment") {
		    ...
		    // std::cerr << elementName << " " << propertyName << " " << valueType << std::endl;
    ...
    if (options_.is_binary()) {
      int offset = 0;
      _is.seekg(streamPos);
      if (static_cast<int>(_is.rdbuf()->sgetc()) == 13) {
	std::cerr << "DOS line endings" << std::endl;
	offset = 14; 
      } else {
	std::cerr << "UNIX line endings" << std::endl;
	offset = 12; 
      }
      _is.seekg(streamPos + offset);
    }
    return true;
}
